...
def get_split(dataset, n_features):
	...
	for index in features:
		for row in dataset:
			...
			groups = test_split(index, row[index], dataset) # row[index]=value is fixed for each iteration
			gini = gini_index(groups, class_values)
			if gini < b_score:
				b_index, b_value, b_score, b_groups = index, row[index], gini, groups  #groups, left and right
